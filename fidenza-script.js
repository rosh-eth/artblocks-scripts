...
function fSeg(a, b, c, e, f, g, h, i) {
  fill(a[0], a[1], a[2]), b ? (stroke(0, 0, 10), swght(w(0.001))) : (stroke(a[0], a[1], a[2]), swght(w(5e-4)));
  const j = [],
    k = [];
...
function pNStps(a, b) {
  return b ? 16 : Math.min(1 / a, wc([0, 0.2, 1, 0.1, 2, 0.15, 4, 0.4, 8, 0.12, 16, 0.03]));
}
const wht = [40, 2, 98],
  dRed = [358, 64, 86],
  red = [358, 80, 82],
  ...
  pbcPolitique = function (a) {
    return a >= V6 ? wc([bgrndNws, 0.5, ppBlue, 0.5]) : wc([bgrndNws, 0.8, ppBlue, 0.2]);
  },
  pcRetro = function () {
...
function draw() {
  ...
  "highAF" === i ? (n *= 0.5) : "med" === i ? (n *= 2.5) : "low" === i && (n *= 5);
  for (let a = h(-0.1); a < h(1.101); a += n) m.push(a);
  m = shffl(m);
  const o = od(0.04),
    p = od(0.15) && a > V1;
  let q = !p && od(0.3);
  const r = od(0.025) && "wM" !== d && "blk" !== d && "dL" !== d && "pG" !== d && "wOC" !== d,
    s = !r && od(0.025) && "wM" !== d && "blk" !== d && "dL" !== d && "pG" !== d && "wOC" !== d,
    t = !p && ("blk" === d || "wOC" === d || (od(0.2) && 70 < j[2] && a >= V5 && "pG" !== d)),
    u = od(0.04),
    v = flwP(c, b, o),
  ...
  for (const b of x) {
    ...
    if (2 <= b.points.length) {
      ...
      fill(f[0], f[1], f[2]), t ? (stroke(0, 0, 15), swght(w(0.001))) : noStroke();
      let c = z4;
      d === z0
        ? (c = w(7e-4))
        : d === z1
